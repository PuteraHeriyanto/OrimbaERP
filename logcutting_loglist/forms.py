...
class LogListForm(forms.ModelForm):
    ...
    partai_id = forms.CharField(label="Partai ID", widget=forms.TextInput(attrs={'class': 'form-control loglist'}))
    tanggal_kedatangan = forms.DateField(label="Tanggal Kedatangan", widget=forms.DateInput(
        attrs={'class': 'form-control datepicker_input loglist', 'placeholder': 'yyyy-mm-dd'}))
    supplier_id = forms.CharField(label="Supplier ID", widget=forms.TextInput(attrs={'class': 'form-control loglist'}))
    grade = forms.CharField(label="Grade", widget=forms.TextInput(attrs={'class': 'form-control loglist'}))
    ...
    class Meta:
        ...
        fields = ['partai_id', 'tanggal_kedatangan', 'supplier_id', 'grade', 'species_id','log_colour',
                  'log_length','diameter_1','diameter_2','average_diameter','volume_bruto','volume_trimming','volume_netto']

        


class LogListDataForm(forms.ModelForm):
    class Meta:
        model = LogList
        # fields = '__all__'
        fields = ['log_id', 'partai_id', 'tanggal_kedatangan','grade','log_colour','log_length',
                  'diameter_1', 'diameter_2', 'average_diameter', 'volume_bruto','volume_trimming','volume_netto']


class LogListInputDataForm(forms.ModelForm):
    class Meta:
        model = LogList
        # fields = '__all__'
        fields = ['log_id', 'partai_id', 'tanggal_kedatangan','supplier_id','grade','species_id','log_colour','log_length',
                  'diameter_1', 'diameter_2', 'volume_trimming']
