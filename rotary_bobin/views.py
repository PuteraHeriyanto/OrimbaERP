...
from .forms import BobinInputForm
...
class BobinCreateView(CreateView):
    model = Bobin
    form_class = BobinInputForm
    template_name = "app/bobin/input_data.html"
    success_url = reverse_lazy('bobin')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = 'Input Data Bobin'
        context["savebtn"] = 'Add to Bobin'
        return context
...
class BobinView(View):
    context = {'segment': 'bobin'}
    ...
    def delete(self, request, pk, action=None):
        bobin = self.get_object(pk)
        bobin.delete()

        redirect_url = None
        if action == 'single':
            messages.success(request, 'Item deleted successfully')
            redirect_url = reverse('bobin')
            ...
            for key in search.split():
                if key.strip():
                    if not filter_params:
                        filter_params = Q(id_bobin_for__icontains=key.strip())
                    else:
                        filter_params |= Q(id_bobin_for__icontains=key.strip())

        bobins = Bobin.objects.filter(filter_params) if filter_params else Bobin.objects.all()

        self.context['bobins'], self.context['info'] = set_pagination(request, bobins)
        if not self.context['bobins']:
            return False, self.context['info']

        return self.context, 'app/bobin/list.html'

    def edit(self, request, pk):
        bobin = self.get_object(pk)

        self.context['bobin'] = bobin
        self.context['form'] = BobinInputForm(instance=bobin)

        return self.context, 'app/bobin/edit.html'

    """ Get Ajax pages """

    def edit_row(self, pk):
        bobin = self.get_object(pk)
        form = BobinInputForm(instance=bobin)
        context = {'instance': bobin, 'form': form}
        return render_to_string('app/bobin/edit_row.html', context)

    """ Common methods """

    def get_object(self, pk):
        bobin = get_object_or_404(Bobin, bobin_id=pk)
        return bobin

    def get_row_item(self, pk):
        bobin = self.get_object(pk)
        edit_row = render_to_string('app/bobin/edit_row.html', {'instance': bobin})
        return edit_row

    def update_instance(self, request, pk, is_urlencode=False):
        bobin = self.get_object(pk)
        form_data = QueryDict(request.body) if is_urlencode else request.POST
        form = BobinInputForm(form_data, instance=bobin)
        if form.is_valid():
            form.save()
            if not is_urlencode:
