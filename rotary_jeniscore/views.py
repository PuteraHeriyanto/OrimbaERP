...
from .forms import JenisCoreInputForm
...
class JenisCoreCreateView(CreateView):
    model = JenisCore
    form_class = JenisCoreInputForm
    template_name = "app/jeniscore/input_data.html"
    success_url = reverse_lazy('jeniscore')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = 'Input Data JenisCore'
        context["savebtn"] = 'Add to JenisCore'
        return context
...
class JenisCoreView(View):
    context = {'segment': 'jeniscore'}
    ...
    def delete(self, request, pk, action=None):
        jeniscore = self.get_object(pk)
        jeniscore.delete()

        redirect_url = None
        if action == 'single':
            messages.success(request, 'Item deleted successfully')
            redirect_url = reverse('jeniscore')
            ...
            for key in search.split():
                if key.strip():
                    if not filter_params:
                        filter_params = Q(id_jenis_core_for__icontains=key.strip())
                    else:
                        filter_params |= Q(id_jenis_core_for__icontains=key.strip())

        jeniscores = JenisCore.objects.filter(filter_params) if filter_params else JenisCore.objects.all()

        self.context['jeniscores'], self.context['info'] = set_pagination(request, jeniscores)
        if not self.context['jeniscores']:
            return False, self.context['info']

        return self.context, 'app/jeniscore/list.html'

    def edit(self, request, pk):
        jeniscore = self.get_object(pk)

        self.context['jeniscore'] = jeniscore
        self.context['form'] = JenisCoreInputForm(instance=jeniscore)

        return self.context, 'app/jeniscore/edit.html'

    """ Get Ajax pages """

    def edit_row(self, pk):
        jeniscore = self.get_object(pk)
        form = JenisCoreInputForm(instance=jeniscore)
        context = {'instance': jeniscore, 'form': form}
        return render_to_string('app/jeniscore/edit_row.html', context)

    """ Common methods """

    def get_object(self, pk):
        jeniscore = get_object_or_404(JenisCore, id_jenis_core=pk)
        return jeniscore

    def get_row_item(self, pk):
        jeniscore = self.get_object(pk)
        edit_row = render_to_string('app/jeniscore/edit_row.html', {'instance': jeniscore})
        return edit_row

    def update_instance(self, request, pk, is_urlencode=False):
        jeniscore = self.get_object(pk)
        form_data = QueryDict(request.body) if is_urlencode else request.POST
        form = JenisCoreInputForm(form_data, instance=jeniscore)
        if form.is_valid():
            form.save()
            if not is_urlencode:
